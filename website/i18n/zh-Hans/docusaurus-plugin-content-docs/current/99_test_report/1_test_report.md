...
6. cleanSession=false与cleanSession=true场景的性能结果存在数倍的差距，原因是BifroMQ提供了基于磁盘而非内存的真正的持久化策略，在宕机或重启时不会丢失数据而影响可靠性。Standalone版本离线消息存储性能受本地磁盘IO瓶颈影响无法得到有效拓展，在集群环境中配合对应的负载策略能得到有效提升。
